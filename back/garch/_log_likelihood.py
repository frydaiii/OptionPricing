...
class LogLikelihooMixin(object):
  def Variance(self, parameters: []):
    ...
    for i in range(length):
      if i == 0:
        ...
        sigma_2[i] = variance(self.log_returns)
      else:
        epsilon_prev = self.log_returns[i - 1] - self.r + 0.5 * sigma_2[i - 1]
        # sigma_2[i] = omega + alpha * self.log_returns[i-1]**2 + beta * sigma_2[i-1]
        sigma_2[i] = (
          omega
          + alpha
          * (epsilon_prev - self.lamb[i] * math.sqrt(sigma_2[i - 1])) ** 2
          + beta * sigma_2[i - 1]
        )
  ...
  def LogLikelihood(self, parameters: []):
    ...
    loglikelihood = -np.sum(-np.log(sigma_2) - self.log_returns**2 / sigma_2)
